...
export function ProficienciesSection({
  ...
  const medium = Boolean(
    getByPath(character.data, "proficiencies.armor.medium")
  );
  const heavy = Boolean(getByPath(character.data, "proficiencies.armor.heavy"));
  const shields = Boolean(
    getByPath(character.data, "proficiencies.armor.shields")
  );
